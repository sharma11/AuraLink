    ...
    <section className="py-24 bg-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <h2 className="font-figtree text-[40px] font-normal leading-tight mb-4 text-slate-900">Choose Your Plan</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {plans.map((plan) => (
            <button
              ...
              className={cn(
                "relative p-8 rounded-2xl text-left transition-all border-2 bg-white",
                selectedPlan === plan.level
                  ? "border-[#156d95] shadow-lg"
            ...
            >
              ...
              <div className="mb-6">
                <h3 className="font-figtree text-2xl font-medium mb-2 text-slate-900">{plan.name}</h3>
                <div className="flex items-baseline gap-1">
                  <span className="font-figtree text-4xl font-medium text-slate-900">
                    ${isYearly ? plan.price.yearly : plan.price.monthly}
                  </span>
                  <span className="font-figtree text-lg text-slate-600">/{isYearly ? "year" : "month"}</span>
                </div>
              </div>
              <div
                className={cn(
                  "w-full py-3 px-6 rounded-full font-figtree text-lg transition-all text-center",
                  selectedPlan === plan.level ? "bg-[#156d95] text-white shadow" : "bg-gray-50 text-slate-900",
                )}
              >
            ...
            <div className="min-w-[768px]">
              ...
              <div className="flex items-center p-6 bg-white border-b border-gray-200">
                <div className="flex-1">
                  <h3 className="font-figtree text-xl font-medium text-slate-900">Features</h3>
                </div>
                <div className="flex items-center gap-8">
                  {plans.map((plan) => (
                    <div key={plan.level} className="w-24 text-center font-figtree text-lg font-medium text-slate-900">
                      {plan.name}
                    </div>
                  ))}
                </div>
        ...
        <div className="mt-12 text-center">
          <button className="relative inline-block overflow-hidden bg-[#156d95] text-white px-[18px] py-[15px] rounded-full font-figtree text-lg hover:rounded-2xl transition-all">
            <span className="pointer-events-none absolute inset-0 opacity-40 mix-blend-screen animate-noise-bg" />
